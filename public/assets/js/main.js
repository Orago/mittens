...
document.addEventListener('keydown', function(event) {/*Nav bar listener*/
  if (event.code == 'KeyI' && (event.ctrlKey || event.metaKey)) {
    if (localStorage.getItem("nav") == "false"){openNav(); /* Messy way to open navigation bar*/document.getElementById("mySearch").focus();localStorage.setItem("nav", "true");
    }else {closeNav(); /* Messy way to open navigation bar*/localStorage.setItem("nav", "false");document.getElementById("mySearch").blur();}}});
